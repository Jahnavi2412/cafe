...
def addEmployee(data):
    try:
        cursor.execute("INSERT INTO employee (emp_name, emp_email, emp_password) VALUES (%s, %s, %s)", data)
        db.commit()
        return True
    except Exception as e:
        print(e)
...
def updateEmployee(data):
    try:
        cursor.execute("UPDATE employee SET emp_name=%s, emp_email=%s, emp_password=%s WHERE id=%s", data)
        db.commit()
        return True
    except Exception as e:
        print(e)
